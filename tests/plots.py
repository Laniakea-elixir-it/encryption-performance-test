    ...
    for directory in os.listdir(basedir):
        ...
        try:
            with open(f'{directory}/{time_metrics_file}','r') as f:
                jobs_info = json.load(f)
                time = [float(v['runtime_raw_value']) for k,v in jobs_info.items()] # includes all WF, also fastqc.
                #time = [float(v['runtime_raw_value']) for k,v in jobs_info.items() if v['tool_id']=="toolshed.g2.bx.psu.edu/repos/iuc/rgrnastar/rna_star/2.7.8a+galaxy0"]
                times += time
...
def boxplot(data, column, ax=0, title='', title_fontsize=40, figsize=None, showfliers=False, notch=False, xlabel=None, pvalues=True):
